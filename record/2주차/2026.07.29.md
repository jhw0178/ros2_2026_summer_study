...
##### 실행 시에는 moveit_test.luanch.py 실행
    $ ros2 launch tf2_basic moveit_test.launch.py
##### launch 파일의 노드 이름 매칭
    moveit_py_node = Node(
        package="tf2_basic",
...
### 3. open_manipulator_x.srdf 수정
##### 3-1. 파일에 임의의 group state를 추가
    <group_state name="my_pose1" group="arm">
        <joint name="joint1" value="1.1"/>
        <joint name="joint2" value="0.2"/>
        <joint name="joint3" value="1.11"/>
        <joint name="joint4" value="0.22"/>
    </group_state>

##### 3-2. 추가한 동작을 동작 코드로 구현
...
##### 4-6. 센서로 인식한 3차원 공간
##### point cloud, boxel map, data map
