...
class Header_pub(Node):
    def __init__(self):
        super().__init__("header_pub") #노드 이름 설정
        # 타이머 등록
        self.create_timer(1 / 10, self.timer_callback) #1초마다 timer_callback 호출
    ...
    def timer_callback(self):
       ...
       msg.stamp = self.get_clock().now().to_msg() #현재 시간
       self.pub.publish(msg) #DDS에 msg를 publish

def main(args=None):
    rclpy.init(args=args) #rmw 활성화
    node = Header_pub() #클래스 불러옴
