...
class TeachManipulator(Node):
    ...
    def __init__(self):
        ...
        self.create_timer(0.3, self.poll_keyboard)        # 키보드를 쓰려면 다음 내용들이 필요함 (이해할 필요까진 없음)
    ...
    def joint_state_callback(self, msg: JointState):
        ...
        available = {
            ...
            for index, nmae in enumerate(msg.name) 
            if index < len(msg.position)
        }
        self._latest_positions = {name: available[name] for name in self.JOINT_NAMES + [self.GRIPPER_JOINT]}
